...
namespace Exato_Price_Table_Module.Entities
{
    public sealed class PriceTable
    {
        ...
        public PriceTable(string name, string? description, bool active, DateTime? validFrom, DateTime? validTo, PrecificationTypeEnum precificationType)
        {
            if (validTo is not null && validTo < validFrom)
                throw new ArgumentOutOfRangeException(nameof(validTo) ,"ValidTo must be greater than ValidFrom.");
